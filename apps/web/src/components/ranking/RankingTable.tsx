...
/**
 * Fila memoizada: cuando llega un RANKING_UPDATE por WebSocket y solo cambian
 * unas pocas entradas, React.memo evita re-renderizar las filas cuyos datos no
 * cambiaron (comparación shallow de props). En leaderboards de 100+ jugadores
 * esto convierte 100+ re-renders en solo los que efectivamente cambiaron.
 */
    ...
    <ul className="space-y-2">
      {entries.map((e) => (
        <RankingRow
          key={e.userId}
          e={e}
          isMe={Boolean(highlightUserId && e.userId === highlightUserId)}
        />
      ))}
    </ul>
  );
